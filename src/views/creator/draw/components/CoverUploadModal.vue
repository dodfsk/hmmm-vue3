<template>
	...
	>
		...
		>
			<n-upload-dragger style="height: 192px">
		...
		<n-space justify="end">
			<n-button type="primary" :disabled="!isClipperReady" v-debounce:click="handleUploadCut">剪裁上传</n-button>
			<!-- <n-button  type="info"   
...
const putUploadFile = async ({ file }: { file: UploadFileInfo }) => {
...
const handleUpload = async (type: string) => {
	...
	//文件名为图片原始文件名+时间戳
	preSignInfo.fileName = originalFile.value!.name + '_' + new Date().getTime() + '.webp'
	console.log('preSignInfo.fileName', preSignInfo.fileName)
